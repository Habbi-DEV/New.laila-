    ...
    <div className="min-h-screen bg-softgray">
      <header className="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-bordergray/70">
        <div className="max-w-md mx-auto px-5 h-14 flex items-center justify-between">
          ...
          {title && <span className="font-serif text-sm text-ink/70">{title}</span>}
        </div>
      </header>
          ...
          {tabs.map(({ to, label, icon: Icon, primary }) => {
            ...
            return (
              <button key={to} onClick={() => navigate(to)} className="tap flex flex-col items-center justify-center gap-0.5">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${primary ? 'bg-burgundy text-white shadow-lift' : active ? 'bg-burgundy/10 text-burgundy' : 'text-ink/40'}`}>
                  <Icon size={18} strokeWidth={primary ? 2.2 : 1.7} />
                </div>
                <span className={`text-[9px] tracking-wide ${active || primary ? 'text-burgundy font-medium' : 'text-ink/40'}`}>{label}</span>
              </button>
            );
          })}
        </div>
